...
class MockTest extends StatefulWidget {
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.of(context).pushNamed('/mockTestMaterial');
                },
                child: Text('OK'),
              ),
            ],
  ...
  Future<void> UpdateUserProgress() async {
    ...
    if (userProgressSnapshot.exists) {
      ...
      var score;
      for (String scoreName in mockTestScore) {
        score += userProgressSnapshot.get(scoreName);
      }

      userProgressDoc.update({
        'Mocktesttaken': testTaken + 1,
        'mockAverageScore': ((score * 100) / mockTestScore.length).round(),
        'index': index,
      });
    }
...
class MockTestState extends State<MockTest> {
  ...
  void initState() {
    super.initState();
    if (widget.isTimerUp || widget.index == 24) {
