...
class MockTestMaterial extends StatefulWidget {
  const MockTestMaterial({super.key});

  @override
  State<MockTestMaterial> createState() => _MockTestMaterialState();
}

class _MockTestMaterialState extends State<MockTestMaterial> {
  var selected = 0;
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
        ...
        .then((snapshot) {
      setState(() {
        TestName = List<String>.from(snapshot.data()!['MockTest'] ?? []);
        TestFlag = List<String>.from(snapshot.data()!['MockTestFlag'] ?? []);
        TestScore = List<String>.from(snapshot.data()!['MockTestScore'] ?? []);
      });
    });
    UpdateUserProgress();
    updateUserData();
...
// Check for missing flags and update user's progress data
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('Note'),
            content: Text(
                'You need to Score 18 Points Out of 24 Points in  45 min  to Pass the Test\n\nClick On \"OK\" to Start Test'),
            actions: [
              TextButton(
                onPressed: () {
                  mockTest(name, Flag, Score);
                },
                child: Text('OK'),
              ),
            ],
  ...
  void mockTest(String test, String flag, testScore) {
    print(test);
                                                ...
                                                children: [
                                                  ...
                                                  Row(
                                                    children: [
                                                      ...
                                                      Container(
                                                        child: Text(
                                                          '  Score ${snapshot.data!.get(TestScore[i])}/24',
                                                          style: TextStyle(
                                                              color: snapshot
                                                                          .data!
                                                                          .get(TestScore[
